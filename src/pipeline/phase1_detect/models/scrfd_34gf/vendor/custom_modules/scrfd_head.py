# Vendored from deepinsight/insightface, detection/scrfd/mmdet/models/dense_heads/scrfd_head.py
# Source: https://github.com/deepinsight/insightface/blob/80e20d868f25344051ae30729257953ed4d3bc43/detection/scrfd/mmdet/models/dense_heads/scrfd_head.py
# Commit: 80e20d868f25344051ae30729257953ed4d3bc43 (fetched 2026-08-12)
# License: Apache-2.0 (see ../LICENSE)
#
# Adaptations from the original file (content otherwise verbatim, including
# original comments/dead code left in place for fidelity):
#   - Imports rewritten from this fork's repo-relative `mmdet.core`/`..builder`/
#     `.anchor_head` etc. to the equivalent absolute paths in a plain
#     `pip install mmdet` tree, since we vendor this one file rather than the
#     fork's entire `mmdet` package.
#   - `distance2kps`/`kps2distance` imported from our vendored
#     `kps_transforms.py` instead of `mmdet.core`, since vanilla mmdet.core
#     does not define these two SCRFD-specific functions.
#
# `SCRFDHead` (registered under mmdet's `HEADS` registry, referenced by
# scrfd_34g.py's `model.bbox_head.type='SCRFDHead'`) does not exist in
# vanilla mmdetection at all, so the whole file is vendored, unlike e.g.
# `PAFPN` (this fork's neck, which vanilla mmdet already ships unchanged and
# is therefore not vendored here -- mmdet's own registered `PAFPN` is used).
import numpy as np
import torch
...
from mmdet.core import (anchor_inside_flags, bbox2distance, bbox_overlaps,
                        ...
                        reduce_mean, unmap)
# `distance2kps`/`kps2distance` are SCRFD-specific additions absent from
# vanilla mmdet.core -- pulled from our own vendored slice instead of the
# fork's full mmdet.core.bbox.transforms (see kps_transforms.py docstring).
from .kps_transforms import distance2kps, kps2distance
