# Vendored, adapted from Krasjet-Yu/YOLO-FaceV2's models/yolo.py.
# Source: https://github.com/Krasjet-Yu/YOLO-FaceV2/blob/2c125edc693a2fbb9d225a023a9514abef9ecb59/models/yolo.py
# No LICENSE file upstream — see ../NOTICE.md before any public release.
#
# Adaptations (mechanical, not a logic change):
#   1. The `models.common` / `models.attention.*` / `models.experimental`
#      imports below are rewritten to relative (`.common`, `.attention.*`,
#      `.experimental`) — same-package references, safe to nest.
#   2. The three `utils.*` imports are kept ABSOLUTE, verbatim — `models`
#      and `utils` are sibling top-level packages in upstream's repo
#      layout, and the pretrained checkpoint was pickled with classes
#      qualified as bare `models.yolo.Model` etc. `convert.py` (the
#      one-time .pt -> ONNX conversion script that is the only caller of
#      this file — see ../NOTICE.md) adds this vendor/ directory to
#      `sys.path` before importing anything here so those bare names
#      resolve correctly at both import time and `torch.load()`-unpickle
#      time — see `models/experimental.py`'s header and ../NOTICE.md for
#      the full explanation. A relative `..utils` import would be
#      flat-out wrong here too: `models` has no parent package once
#      loaded as a bare top-level package, so a two-dot relative import
#      would raise ImportError.
#   3. Dropped `sys.path.append('./')` — a script-mode-only convenience
#      for running `python models/yolo.py` directly; superseded by
#      `convert.py`'s own explicit `sys.path` handling.
#   4. Dropped the `if __name__ == '__main__':` CLI block at the bottom
