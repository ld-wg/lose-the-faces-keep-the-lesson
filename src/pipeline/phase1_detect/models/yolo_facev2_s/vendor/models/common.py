# Vendored, TRIMMED subset of Krasjet-Yu/YOLO-FaceV2's models/common.py.
# Source: https://github.com/Krasjet-Yu/YOLO-FaceV2/blob/2c125edc693a2fbb9d225a023a9514abef9ecb59/models/common.py
# No LICENSE file upstream — see ../NOTICE.md before any public release.
#
# Trim (documented, not a logic change): the upstream file also defines
# `NMS`, `autoShape`, `Detections`, and `Classify` — demo/convenience
# wrappers (rich-display, autoshaping arbitrary input types, classifier
# head) that pull in pandas/requests/Pillow/torch.cuda.amp as *module-level*
# imports even though `convert.py` (the only caller of this file — see
# ../NOTICE.md) never calls them. Dropped here, along with those four
# now-unused imports, so this vendored slice's only hard dependency is
# `torch` (matches pyproject.toml's `yolo-facev2-convert` extra). Every
# class body below (the model-definition code that actually matters) is
# verbatim.
